...
func ConstPointerCast(
	...
	rv.C = C.LLVMConstPointerCast(v.C, t.C)
	return
}
...
func PrevInstruction(
	...
	rv.C = C.LLVMGetPreviousInstruction(v.C)
	return
}
...
func InlineAsm(
	t Type,
	asmString, constraints string,
	hasSideEffects, isAlignStack bool,
	...
	rv.C = C.LLVMGoGetInlineAsm(
		t.C,
		casm,
		C.size_t(len(asmString)),
		cconstraints,
		C.size_t(len(constraints)),
		boolToLLVMBool(hasSideEffects),
		boolToLLVMBool(isAlignStack),
		C.LLVMInlineAsmDialect(dialect),
		boolToLLVMBool(canThrow),
	)
	return
}
...
func (b Builder) CreateInvoke(
	t Type,
	fn Value,
	args []Value,
	then, catch BasicBlock,
	name string,
) (rv Value) {
	cname := C.CString(name)
	defer C.free(unsafe.Pointer(cname))
...
func (b Builder) CreateCatchSwitch(
	parentPad Value,
	unwindBB BasicBlock,
	numHandlers int,
	name string,
...
func (b Builder) CreateAtomicRMW(
	op AtomicRMWBinOp,
	ptr, val Value,
	ordering AtomicOrdering,
	singleThread bool,
) (v Value) {
	v.C = C.LLVMBuildAtomicRMW(
		b.C,
		C.LLVMAtomicRMWBinOp(op),
		ptr.C,
		val.C,
		C.LLVMAtomicOrdering(ordering),
		boolToLLVMBool(singleThread),
	)
	return
}

func (b Builder) CreateAtomicCmpXchg(
	ptr, cmp, newVal Value,
	successOrdering, failureOrdering AtomicOrdering,
	singleThread bool,
) (v Value) {
	v.C = C.LLVMBuildAtomicCmpXchg(
		b.C,
		ptr.C,
		cmp.C,
		newVal.C,
		C.LLVMAtomicOrdering(successOrdering),
		C.LLVMAtomicOrdering(failureOrdering),
		boolToLLVMBool(singleThread),
	)
	return
}
